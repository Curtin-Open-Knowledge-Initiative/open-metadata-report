WITH affiliations_processed as (
  SELECT
    ...
    affiliation.GridId
  FROM `{tables[source].get('Affiliations')}` as affiliation
),
...
fields_of_study as (
    SELECT
        ...
        ARRAY(SELECT AS STRUCT
         ...
         FROM `{tables[source].get('FieldOfStudyExtendedAttributes')}` as extended
         WHERE extended.FieldOfStudyId = fields.FieldOfStudyId
        ) as extended
    FROM `{tables[source].get('FieldsOfStudy')}` as fields
)
...
FROM `{selector[source]}` as papers

-- Abstract
LEFT JOIN `{tables[source].get('PaperAbstractsInvertedIndex')}` as abstracts ON abstracts.PaperId = papers.PaperId

-- Journal
LEFT JOIN `{tables[source].get('Journals')}` as journal ON journal.JournalId = papers.JournalId

-- ConferenceInstance
LEFT JOIN `{tables[source].get('ConferenceInstances')}` as conferenceInstance ON conferenceInstance.ConferenceInstanceId = papers.ConferenceInstanceId

-- ConferenceSeries
LEFT JOIN `{tables[source].get('ConferenceSeries')}` as conferenceSeries ON conferenceSeries.ConferenceSeriesId = papers.ConferenceSeriesId
...
LEFT JOIN (SELECT
            ...
            LEFT JOIN `{tables[source].get('PaperFieldsOfStudy')}` as paperFields on papers.PaperId = paperFields.PaperId
...
LEFT JOIN (SELECT
            ...
            FROM `{tables[source].get('Papers')}` as papers
            LEFT JOIN `{tables[source].get('PaperAuthorAffiliations')}` as paperAuthorAffiliations on paperAuthorAffiliations.PaperId = papers.PaperId
...
LEFT JOIN (SELECT
            ...
            FROM `{tables[source].get('PaperExtendedAttributes')}`
...
LEFT JOIN (SELECT
            ...
            FROM `{tables[source].get('PaperResources')}`
...
LEFT JOIN (SELECT
            ...
            FROM `{tables[source].get('PaperUrls')}`
...
LEFT JOIN (SELECT
            ...
            FROM `{tables[source].get('PaperMeSH')}`
